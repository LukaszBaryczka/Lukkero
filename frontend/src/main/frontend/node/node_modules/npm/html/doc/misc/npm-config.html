...
<body>
	<div id="wrapper">
		...
		<p>
			...
			<code>--flag</code>
			parameter that is at the <em>end</em> of the command will be given
			the value of
			<code>true</code>
			.
		</p>
		...
		<p>
			...
			will work the same. However, please note that inside <a
				href="/misc/scripts">npm-scripts</a> npm will set it&#39;s own
...
, &quot;scripts&quot; : { &quot;start&quot; : &quot;node server.js&quot; } }
		...
		<pre>
			<code>http.createServer(...).listen(process.env.npm_package_config_port)
</code>
		</pre>
		<p>then the user could change the behavior by doing:</p>
		<pre>
			<code>npm config set foo:port 80
</code>
		</pre>
		<p>
...
ca[]=&quot;...&quot;
		...
		<p>The maximum time (in seconds) to keep items in the registry
			cache before re-checking against the registry.</p>
		<p>
		...
		<p>The minimum time (in seconds) to keep items in the registry
			cache before re-checking against the registry.</p>
		<p>
			Note that no purging is done unless the
			<code>npm cache clean</code>
		...
		<p>
			...
			,
			<code>uninstall</code>
			. This is NOT currently honored by network related commands, eg
			<code>dist-tags</code>
			,
			<code>owner</code>
			,
		...
		<p>Any logs of a higher level than the setting are shown. The
			default is &quot;warn&quot;, which shows warn and error output.</p>
		...
		<p>Whether or not to include proprietary extended attributes in
			the tarballs created by npm.</p>
		<p>Unless you are expecting to unpack package tarballs with
			something other than npm -- particularly a very outdated tar
			implementation -- leave this as true.</p>
	...
	<p id="footer">npm-config &mdash; npm@4.4.4</p>